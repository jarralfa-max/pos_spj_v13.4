...
async def pending_orders(
    ...
    """Devuelve pedidos WhatsApp pendientes para el tablero POS/delivery."""
    conn = _connect()
    try:
        ...
        where = ["COALESCE(v.estado, '') IN ('pendiente_wa', 'pendiente', 'confirmado', 'en_preparacion')"]
        params: list = []
        if has_canal:
        ...
        for row in rows:
            ...
            if "nombre" in detalle_cols:
                detail_select.append("COALESCE(d.nombre, '') AS nombre")
            else:
                detail_select.append("COALESCE(p.nombre, '') AS nombre")
            if "unidad" in detalle_cols:
                detail_select.append("COALESCE(d.unidad, p.unidad, 'kg') AS unidad")
            else:
