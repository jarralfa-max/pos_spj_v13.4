    ...
    def _insert_inbox_for_roles(
        ...
        *,
        ...
        for emp in empleados:
            self._insert_one_inbox(emp, tipo, titulo, cuerpo, datos, sucursal_id)
            ...
            if "empleado_id" in cols:
                self.db.execute("""
                    INSERT INTO notification_inbox
                        (empleado_id, tipo, titulo, cuerpo, datos, sucursal_id, leido, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, 0, datetime('now'))
                """, (emp.get("id"), tipo, titulo, cuerpo, datos_json, sucursal_id))
            elif "usuario" in cols:
                self.db.execute("""
                    INSERT INTO notification_inbox
                        (usuario, tipo, titulo, cuerpo, sucursal_id, leido, created_at)
                    VALUES (?, ?, ?, ?, ?, 0, datetime('now'))
                """, (emp.get("usuario") or emp.get("nombre") or "admin", tipo, titulo, cuerpo, sucursal_id))
            else:
