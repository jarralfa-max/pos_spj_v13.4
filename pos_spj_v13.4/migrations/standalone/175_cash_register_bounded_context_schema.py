...
DDL = (
    f"""CREATE TABLE IF NOT EXISTS cash_registers (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), branch_id TEXT NOT NULL CHECK({U('branch_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_shift_assignments (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), shift_id TEXT NOT NULL REFERENCES cash_shifts(id) ON DELETE CASCADE,
    ...
    f"""CREATE TABLE IF NOT EXISTS payment_records (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), sale_id TEXT NOT NULL CHECK({U('sale_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS payment_allocations (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), payment_record_id TEXT NOT NULL REFERENCES payment_records(id) ON DELETE CASCADE,
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_count_denominations (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), count_id TEXT NOT NULL REFERENCES cash_counts(id) ON DELETE CASCADE,
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_deposit_preparations (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), handover_id TEXT NOT NULL UNIQUE REFERENCES cash_handovers(id),
        branch_id TEXT NOT NULL CHECK({U('branch_id')}), amount TEXT NOT NULL CHECK(CAST(amount AS NUMERIC)>0),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_handover_denominations (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), handover_id TEXT NOT NULL REFERENCES cash_handovers(id) ON DELETE CASCADE,
        denomination_id TEXT NOT NULL REFERENCES cash_denominations(id),
        denomination TEXT NOT NULL CHECK(CAST(denomination AS NUMERIC)>0),
        quantity INTEGER NOT NULL CHECK(quantity>=0), subtotal TEXT NOT NULL CHECK(CAST(subtotal AS NUMERIC)>=0),
        UNIQUE(handover_id,denomination_id))""",
    f"""CREATE TABLE IF NOT EXISTS cash_handover_confirmations (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), handover_id TEXT NOT NULL REFERENCES cash_handovers(id),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_authorization_grants (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), requested_by TEXT NOT NULL CHECK({U('requested_by')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_refund_executions (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), refund_id TEXT NOT NULL CHECK({U('refund_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS drawer_open_events (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), drawer_id TEXT NOT NULL REFERENCES cash_drawers(id),
        shift_id TEXT NOT NULL REFERENCES cash_shifts(id), branch_id TEXT NOT NULL CHECK({U('branch_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_audit_log (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), action TEXT NOT NULL CHECK(trim(action)<>''),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_outbox (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), event_id TEXT NOT NULL UNIQUE REFERENCES cash_domain_events(id),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_processed_operations (
        operation_id TEXT NOT NULL PRIMARY KEY CHECK({U('operation_id')}), operation_type TEXT NOT NULL CHECK(trim(operation_type)<>''),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_print_jobs (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), operation_id TEXT NOT NULL UNIQUE CHECK({U('operation_id')} AND operation_id<>id),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_print_audit (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), print_id TEXT NOT NULL CHECK({U('print_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_sync_envelopes (
        id TEXT NOT NULL PRIMARY KEY CHECK({U('id')}), outbox_id TEXT NOT NULL UNIQUE REFERENCES cash_outbox(id),
