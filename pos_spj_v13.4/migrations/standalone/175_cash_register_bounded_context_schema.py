...
DDL = (
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_print_jobs (
        ...
        printer_id TEXT NOT NULL CHECK(trim(printer_id)<>''), document_type TEXT NOT NULL CHECK(document_type IN ('X_CUT','Z_CUT','MOVEMENT_RECEIPT','SAFE_DROP','HANDOVER','REFUND')),
        entity_id TEXT NOT NULL CHECK({U('entity_id')}), branch_id TEXT NOT NULL CHECK({U('branch_id')}),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_print_audit (
        ...
        entity_id TEXT NOT NULL CHECK({U('entity_id')}), document_type TEXT NOT NULL CHECK(document_type IN ('X_CUT','Z_CUT','MOVEMENT_RECEIPT','SAFE_DROP','HANDOVER','REFUND')),
        output_format TEXT NOT NULL CHECK(output_format IN ('HTML','ESC_POS')),
