...
DDL = (
    f"""CREATE TABLE IF NOT EXISTS cash_settings (
      id TEXT PRIMARY KEY CHECK({U('id')}), setting_key TEXT NOT NULL CHECK(trim(setting_key)<>''),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_denominations (
      id TEXT PRIMARY KEY CHECK({U('id')}), currency_code TEXT NOT NULL CHECK(length(currency_code)=3),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_payment_methods (
      id TEXT PRIMARY KEY CHECK({U('id')}), code TEXT NOT NULL, display_name TEXT NOT NULL,
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_operation_limits (
      id TEXT PRIMARY KEY CHECK({U('id')}), operation_type TEXT NOT NULL,
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_movement_reasons (
      id TEXT PRIMARY KEY CHECK({U('id')}), code TEXT NOT NULL, display_name TEXT NOT NULL CHECK(trim(display_name)<>''),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_difference_policies (
      id TEXT PRIMARY KEY CHECK({U('id')}), tolerance_amount TEXT NOT NULL CHECK(CAST(tolerance_amount AS NUMERIC)>=0),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_permission_profiles (
      id TEXT PRIMARY KEY CHECK({U('id')}), name TEXT NOT NULL UNIQUE, description TEXT NOT NULL DEFAULT '',
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_notification_jobs (
      id TEXT PRIMARY KEY CHECK({U('id')}), source_event_id TEXT NOT NULL REFERENCES cash_domain_events(id),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_notification_attempts (
      id TEXT PRIMARY KEY CHECK({U('id')}), job_id TEXT NOT NULL REFERENCES cash_notification_jobs(id),
    ...
    f"""CREATE TABLE IF NOT EXISTS cash_in_app_alerts (
      id TEXT PRIMARY KEY CHECK({U('id')}), notification_job_id TEXT NOT NULL UNIQUE REFERENCES cash_notification_jobs(id),
