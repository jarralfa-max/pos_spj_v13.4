...
# backend/infrastructure/db/connection.py — SPJ POS (stable edition)
"""
...
# MOVIDO desde `core/db/connection.py` a la capa canónica.
#
# Ojo con esta ruta: en su ubicación anterior (`core/db/`) el `../../` daba la
# raíz del paquete. Aquí, tres niveles abajo (`backend/infrastructure/db/`),
# ese mismo `../../` apuntaría a `backend/` y la base de datos se crearía en
# `backend/data/` — otra BD, silenciosamente. Por eso son tres niveles.
...
DB_PATH = CANONICAL_DB_PATH  # alias legacy
_TIMEOUT = 5.0
_MAX_RETRY = 5
...
def wrap(conn) -> "DatabaseWrapper":
    ...
    Muchos servicios legacy importan `wrap` desde `backend.infrastructure.db.connection`.
