"""Canonical Fidelidad/Loyalty RBAC adapter over the live application
session.

Mirrors ``backend/application/sales/session_authorization.py``'s
``SalesSessionPermissionChecker`` exactly. This is the **real**
``PermissionChecker`` the composition root (`core/app_container.py`) wires
into ``LoyaltyAuthorizationPolicy`` (never a mock).
...
class LoyaltySessionPermissionChecker:
    ...
    has an active branch, and directly holds the canonical
    `GROWTH_ENGINE.accion` code (``session.tiene_permiso``). No session /
    inactive session / user mismatch / no active branch / no
    ``tiene_permiso`` → deny.
    """
