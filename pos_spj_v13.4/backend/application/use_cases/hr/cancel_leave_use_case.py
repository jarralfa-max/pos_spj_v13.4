"""Cancel HR leave requests."""

from __future__ import annotations

from datetime import UTC, datetime

from backend.application.commands.hr_commands import CancelLeaveCommand
...
class CancelLeaveUseCase(BaseUseCase[CancelLeaveCommand]):
    name = "CancelLeaveUseCase"
    ...
    def execute(self, command: CancelLeaveCommand) -> UseCaseResult:
        ...
        if previous not in (LeaveStatus.DRAFT, LeaveStatus.PENDING, LeaveStatus.APPROVED):
            raise ValueError("leave request cannot be cancelled from its current state")
        ...
        event = create_domain_event(event_name=EventName.LEAVE_CANCELLED, operation_id=command.operation_id, entity_id=leave_request.id, branch_id=leave_request.branch_id, user_id=command.user_id, user_name=command.user_name, source_module="HR", payload={"employee_id": leave_request.employee_id, "reason": command.reason.strip()})
        if self._event_bus is not None:
            self._event_bus.publish(event)
        record_hr_audit(self._audit_sink, action="LEAVE_CANCELLED", operation_id=command.operation_id, entity_id=leave_request.id, actor_user_id=command.user_id, branch_id=leave_request.branch_id, metadata={"employee_id": leave_request.employee_id, "reason": command.reason.strip()})
