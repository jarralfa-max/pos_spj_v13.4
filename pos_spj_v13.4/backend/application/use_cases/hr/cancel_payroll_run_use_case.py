"""Cancel unpaid canonical payroll runs."""

from __future__ import annotations

from datetime import UTC, datetime

from backend.application.commands.hr_commands import CancelPayrollRunCommand
...
class CancelPayrollRunUseCase(BaseUseCase[CancelPayrollRunCommand]):
    name = "CancelPayrollRunUseCase"
    ...
    def execute(self, command: CancelPayrollRunCommand) -> UseCaseResult:
        command.validate_context()
        self._authorizer.require(command.user_id, "hr.payroll.cancel")
        payroll_run = self._payroll_repository.get_run(command.payroll_run_id)
        if payroll_run is None:
            raise ValueError("payroll run not found")
        self._policy.ensure_can_cancel(payroll_run.status)
        ...
        event = create_domain_event(event_name=EventName.PAYROLL_CANCELLED, operation_id=command.operation_id, entity_id=payroll_run.id, branch_id=payroll_run.branch_id, user_id=command.user_id, user_name=command.user_name, source_module="HR", payload={"reason": command.reason.strip()})
        if self._event_bus is not None:
            self._event_bus.publish(event)
        record_hr_audit(self._audit_sink, action="PAYROLL_CANCELLED", operation_id=command.operation_id, entity_id=payroll_run.id, actor_user_id=command.user_id, branch_id=payroll_run.branch_id, metadata={"reason": command.reason.strip()})
