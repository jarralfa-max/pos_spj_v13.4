...
from backend.application.commands.hr_commands import ApproveLeaveCommand
...
class ApproveLeaveUseCase(BaseUseCase[ApproveLeaveCommand]):
    name = "ApproveLeaveUseCase"
    ...
    def execute(self, command: ApproveLeaveCommand) -> UseCaseResult:
        ...
        if leave_request.leave_type == LeaveType.VACATION:
            available = self._leave_repository.get_available_days(employee_id=leave_request.employee_id, branch_id=leave_request.branch_id, leave_type=leave_request.leave_type)
            if available < leave_request.requested_days:
                raise InsufficientLeaveBalanceError("insufficient leave balance")
            self._leave_repository.debit_balance(employee_id=leave_request.employee_id, branch_id=leave_request.branch_id, leave_type=leave_request.leave_type, days=leave_request.requested_days)
        leave_request.status = LeaveStatus.APPROVED
        leave_request.approved_by_user_id = command.user_id
        ...
        self._leave_repository.add_history(leave_request_id=leave_request.id, previous_status=previous, new_status=leave_request.status, actor_user_id=command.user_id, reason="Aprobación", operation_id=command.operation_id)
        event = create_domain_event(event_name=EventName.LEAVE_APPROVED, operation_id=command.operation_id, entity_id=leave_request.id, branch_id=leave_request.branch_id, user_id=command.user_id, user_name=command.user_name, source_module="HR", payload={"employee_id": leave_request.employee_id})
        if self._event_bus is not None:
            self._event_bus.publish(event)
        record_hr_audit(self._audit_sink, action="LEAVE_APPROVED", operation_id=command.operation_id, entity_id=leave_request.id, actor_user_id=command.user_id, branch_id=leave_request.branch_id, metadata={"employee_id": leave_request.employee_id})
