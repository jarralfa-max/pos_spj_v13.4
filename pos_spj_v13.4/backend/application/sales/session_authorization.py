"""Canonical Sales/POS RBAC adapter over the live application session.

Mirrors backend/application/customers/session_authorization.py's
`CustomerSessionPermissionChecker` / backend/application/inventory/session_authorization.py's
`InventorySessionPermissionChecker` exactly. This is the **real**
``PermissionChecker`` the composition root (`core/app_container.py`) wires
into `SalesAuthorizationPolicy` (never a mock).

Identity is validated against the live session, so branch changes, logout and
permission refreshes are observed on every check. A missing session, an
...
class SalesSessionPermissionChecker:
    """`PermissionChecker` over the live session for `SalesAuthorizationPolicy`.

    Grants only when the session is active, belongs to the requesting user,
    has an active branch, and directly holds the canonical `POS.accion` code
    (``session.tiene_permiso``). No session / inactive session / user
