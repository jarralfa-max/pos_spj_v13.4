"""Canonical inventory RBAC adapter over the live application session.

The live catalog seeds roles directly with the granular `INVENTARIO.accion`
codes (see `core/security/permission_catalog.py` and
`backend/application/inventory/permissions.py`) — the same vocabulary used by
`rol_permisos`, the Configuración permission matrix and every other module.
This is the **real** ``PermissionChecker`` production wires into
``InventoryUseCaseFactory`` (never a mock — §23); there is no legacy code
translation layer.
...
class InventorySessionPermissionChecker:
    ...
    Grants only when the session is active, belongs to the requesting user, has
    an active branch, and directly holds the canonical `INVENTARIO.accion` code
    (``session.tiene_permiso``). No session / inactive session / user mismatch /
    no active branch / no ``tiene_permiso`` → deny.
    """
