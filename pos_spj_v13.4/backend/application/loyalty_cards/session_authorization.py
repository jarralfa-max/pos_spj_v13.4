"""Canonical Loyalty Cards RBAC adapter over the live application session.

Mirrors ``backend/application/loyalty/session_authorization.py`` exactly.
This is the **real** ``PermissionChecker`` the composition root
(`core/app_container.py`) wires into ``LoyaltyCardsAuthorizationPolicy``
(never a mock).
"""
...
class LoyaltyCardsSessionPermissionChecker:
    ...
    `TARJETAS_FIDELIDAD.accion` code (``session.tiene_permiso``).
    """
