...
def _db():
    ...
    c.executescript(
        """
        CREATE TABLE productos (id INTEGER PRIMARY KEY, nombre TEXT, unidad TEXT DEFAULT 'kg', precio_compra REAL DEFAULT 0, activo INTEGER DEFAULT 1, is_active INTEGER DEFAULT 1, tipo_producto TEXT DEFAULT 'simple');
        CREATE TABLE product_recipes (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre_receta TEXT, product_id INTEGER, tipo_receta TEXT, unidad_base TEXT DEFAULT 'kg', peso_promedio_kg REAL DEFAULT 1, is_active INTEGER DEFAULT 1, activa INTEGER DEFAULT 1, piece_product_id INTEGER, validates_at TEXT, created_at TEXT, total_rendimiento REAL DEFAULT 0, total_merma REAL DEFAULT 0);
        CREATE TABLE product_recipe_components (id INTEGER PRIMARY KEY AUTOINCREMENT, recipe_id INTEGER, component_product_id INTEGER, rendimiento_pct REAL DEFAULT 0, merma_pct REAL DEFAULT 0, cantidad REAL DEFAULT 0, unidad TEXT DEFAULT 'kg', component_role TEXT DEFAULT '', factor_costo REAL DEFAULT 1.0, tolerancia_pct REAL DEFAULT 2.0, orden INTEGER DEFAULT 0, descripcion TEXT DEFAULT '');
        CREATE TABLE recipe_dependency_graph (id INTEGER PRIMARY KEY AUTOINCREMENT, parent_recipe_id INTEGER, child_product_id INTEGER, depth INTEGER DEFAULT 1);
        CREATE TABLE producciones (id INTEGER PRIMARY KEY AUTOINCREMENT, receta_id INTEGER, producto_base_id INTEGER, cantidad_base REAL, unidad_base TEXT, usuario TEXT, sucursal_id INTEGER, notas TEXT, estado TEXT, fecha TEXT, operation_id TEXT UNIQUE);
        CREATE TABLE produccion_detalle (id INTEGER PRIMARY KEY AUTOINCREMENT, produccion_id INTEGER, producto_resultante_id INTEGER, cantidad_generada REAL, unidad TEXT, rendimiento_aplicado REAL, tipo TEXT);
