...
class TestP03BomCycleBlocksSale:
    """
    SaleInventoryHandler._deduct_via_resolver() debe lanzar ValueError
    cuando RecipeResolver reporta cycle_detected=True.
    """

    def _make_handler_with_cycle(self):
        """Devuelve un handler cuyo resolver siempre devuelve cycle_detected=True."""
        from core.events.handlers.inventory_handler import SaleInventoryHandler

        inv_mock = MagicMock()
        inv_mock.deduct_stock = MagicMock()

        conn = _mem_db()
        _setup_recipe_schema(conn)

        handler = SaleInventoryHandler(inventory_service=inv_mock, db=conn)

        # Deduction line ficticia para que no sea "no_recipe"
        deduction_line = SimpleNamespace(
            product_id=99,
            quantity=1.0,
            is_virtual=False,
        )
        cycle_explosion = SimpleNamespace(
            cycle_detected=True,
            deductions=[deduction_line],
        )

        return handler, cycle_explosion
    ...
    def test_cycle_detected_raises_value_error(self):
        ...
        cycle_result = SimpleNamespace(
            cycle_detected=True,
            deductions=[SimpleNamespace(product_id=5, quantity=2.0, is_virtual=False)],
        )

        with patch(
            "core.services.recipes.recipe_resolver.RecipeResolver.resolve_for_sale",
            return_value=cycle_result,
        ):
            with pytest.raises(ValueError, match="referencia circular"):
                handler._deduct_via_resolver(
                    product_id=5,
                    sale_qty=2.0,
                    branch_id=1,
                    sale_id="TEST-001",
    ...
    def test_cycle_detected_inventory_not_deducted(self):
        """Cuando hay ciclo, deduct_stock NO debe llamarse."""
        from core.events.handlers.inventory_handler import SaleInventoryHandler
        ...
        with patch(
            ...
            with pytest.raises(ValueError):
                handler._deduct_via_resolver(
                    product_id=5, sale_qty=2.0, branch_id=1,
                    sale_id="T", operation_id="O", user="u", folio="f",
                )

        inv_mock.deduct_stock.assert_not_called()

    def test_no_cycle_normal_deduction_proceeds(self):
        """Sin ciclo, el flujo normal continúa sin lanzar excepciones."""
        from core.events.handlers.inventory_handler import SaleInventoryHandler
        ...
        ok_result = SimpleNamespace(
            ...
            deductions=[SimpleNamespace(product_id=10, quantity=1.5, is_virtual=False)],
        )
        ...
        with patch(
            ...
            # No lanza
            handler._deduct_via_resolver(
                product_id=3, sale_qty=1.0, branch_id=1,
...
# ─────────────────────────────────────────────────────────────────────────────
#  P0-4: INSERT OR IGNORE crea fila en inventario_actual cuando no existe
# ─────────────────────────────────────────────────────────────────────────────
