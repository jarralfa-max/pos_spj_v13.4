...
- El formulario de producto usa un selector (QComboBox con `currentData`) para la
  unidad base y NO un `QLineEdit` de texto.
...
def test_form_unit_is_a_catalog_selector():
    ...
    assert "self.base_unit = QComboBox()" in src
    assert 'self.base_unit.currentData()' in src
