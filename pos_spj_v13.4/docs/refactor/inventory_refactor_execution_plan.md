...
| **INV-26** | Impresión/etiquetas (lote/peso/transferencia/conteo/ajuste): enums `LabelType`/`LabelFormat`, VO `LabelDocument` (Decimal-safe), eventos LABEL_PRINTED/REPRINTED, tabla `inventory_label_print_log` + migración 135, `InventoryPrintGateway` (Protocol + InMemory) + renderers puros + `InventoryLabelPrintService` (permiso LABEL_PRINT/REPRINT, auditoría, reimpresión, evento) | `backend/**` + tests (8) | ✅ |
...
| **INV-28** | **Validación final**: suites dominio/aplicación/integración/UI/arquitectura de Inventario en verde (guardrails 85, integración 252, unit 165), bootstrap limpio desde cero (120 migraciones, cobertura 80.0%, sin errores), diff de regresión contra base pre-corte (`e3720b9~1`) con **cero fallas netas nuevas** (3 tests obsoletos del shim `inventory_service` eliminados; resto de fallas del set legacy pre-existentes e idénticas a base); `module_relocation_map.md` → **MIGRATED**; reporte `inventory_validation_report.md` | `docs/refactor/inventory_validation_report.md` + `module_relocation_map.md` | ✅ |
