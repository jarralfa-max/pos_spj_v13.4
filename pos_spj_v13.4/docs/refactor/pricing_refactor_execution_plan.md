...
| PRC-1 | Seguridad: permisos granulares `PRICING_*` (ver/gestionar precios, aprobar listas, ver costo, override precio mínimo), authorization policy + segregación | `backend/application/pricing/{permissions,authorization}` + tests | ⏳ |
| PRC-2 | Dominio: VO `Money` (Decimal + moneda), `Price`/`Cost`/`Margin` VOs, entidades `PriceList`/`ProductPrice`/`VolumePrice`/`ProductCost`, `MarginPolicy` (precio mínimo/margen objetivo), enums, eventos, excepciones | `backend/domain/pricing/**` + tests | ⏳ |
| PRC-3 | Esquema born-clean: `pricing_schema.py` (`price_list`, `product_price`, `volume_price`, `customer_price_list`, `product_cost`, `price_change_log`; UUIDv7, Money TEXT Decimal, sin REAL) + migración registrada + bootstrap | `infrastructure/db/schema/pricing_schema.py` + migración + tests | ⏳ |
| PRC-4 | Resolución + query services: `PricingResolutionService` (volumen > lista cliente > lista > base, respeta precio mínimo), `ProductPriceQueryService.get_sale_price(...)`, `ProductCostQueryService.get_average_cost(...)`; repositorios | `application/pricing/{queries,services}` + repos + tests | ⏳ |
