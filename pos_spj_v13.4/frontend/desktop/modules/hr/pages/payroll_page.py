...
=======
    HEADERS = ("ID", "Sucursal", "Periodo", "Estado", "Bruto", "Deducciones", "Neto")
    HIDDEN_HEADERS = ("ID",)
>>>>>>> 5f7df5247ec66f7297adb53c0d2e32ee56e33c23
...
=======
                values = [row.payroll_run_id, row.branch_label, row.period, row.status, str(row.gross_amount), str(row.deductions_amount), str(row.net_amount)]
                for column, value in enumerate(values):
                ...
                self._table.set_status_badge(
                    ...
                    3,
                    row.status,
                    status="success" if row.status == "PAID" else "neutral",
                    tooltip="Estado de la corrida de nómina",
                )
            self._pagination.update_state(total_rows=len(rows))
