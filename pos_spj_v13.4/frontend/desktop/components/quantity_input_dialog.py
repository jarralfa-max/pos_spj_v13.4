"""Diálogo estándar de captura de cantidad con teclado numérico tipo calculadora.
...
Reemplaza el uso crudo de QInputDialog.getDouble para capturar cantidades/pesos.
Acepta entrada por teclado físico (báscula/USB) y por el teclado en pantalla
(touch/mouse), útil en el contexto POS.
...
Uso (drop-in, misma forma que QInputDialog.getDouble):
    ...
    cantidad, ok = QuantityInputDialog.get_quantity(
        self, "Cantidad", f"Ingrese la cantidad para {nombre}:",
        decimals=3, unidad="kg")
...
class QuantityInputDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        lbl.setObjectName("quantityInputPrompt")
        lay.addWidget(lbl)
        ...
        self.display.setObjectName("quantityInputDisplay")
        ...
        if unidad:
            u = QLabel(unidad)
            u.setObjectName("quantityInputUnit")
            disp_row.addWidget(u)
        lay.addLayout(disp_row)

        # Sólo pre-carga un valor inicial cuando se edita una cantidad existente
    ...
    def _sync_ok_enabled(self) -> None:
        v = self.valor()
        self.btn_ok.setEnabled(v > 0 and self._minimo <= v <= self._maximo)
